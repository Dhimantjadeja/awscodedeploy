...
function _LaunchCloudFormationStack([string]$instanceType, [string]$keyPair, [bool]$openRDP)
{
    ...
    $stackId = New-CFNStack -StackName "CodeDeploy" -Capability "CAPABILITY_IAM" -Parameter $parameters -TemplateBody $templateBody
    $stackId
}

function _SetupCodeDeployResources([string]$serviceRole)
{    
    _SetupCodeDeployApplication "CodeDeploy" $serviceRole
}
